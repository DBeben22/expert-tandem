...
def generate_massspectra_plot_automatic_labels(input_df,input_annotations, output_filename):
    ...
    spectra = {'m/z': input_df["m/z"], 'intensity': input_df["intensity"]}
    newSpectra = {'m/z': [], 'intensity': []}
    idxSpectra = 0
    for i in range(1, len(spectra['m/z'])):
        diff = spectra['m/z'][i] - spectra['m/z'][i - 1]
        ...
        if diff > norm_mz_diff * diffFactor:
            if diff < norm_mz_diff * 2:
                newSpectra['m/z'].insert(idxSpectra,
                                        spectra['m/z'][i - 1] + diff / 2)
                newSpectra['intensity'].insert(idxSpectra, 0)
                idxSpectra += 1
                # print(norm_mz_diff *diffFactor, diff, norm_mz_diff *diffFactor + diff, norm_mz_diff)
                newSpectra['m/z'].insert(idxSpectra, spectra['m/z'][i])
                newSpectra['intensity'].insert(idxSpectra,
                                               spectra['intensity'][i])
                idxSpectra += 1
            else:
                newSpectra['m/z'].insert(idxSpectra,
                                        spectra['m/z'][i - 1] + norm_mz_diff)
                newSpectra['intensity'].insert(idxSpectra, 0)
                idxSpectra += 1
                newSpectra['m/z'].insert(idxSpectra,
                                        spectra['m/z'][i] - norm_mz_diff)
                newSpectra['intensity'].insert(idxSpectra, 0)
                idxSpectra += 1
                newSpectra['m/z'].insert(idxSpectra, spectra['m/z'][i])
                newSpectra['intensity'].insert(idxSpectra,
    ...
    if mzRange == (0, 0):
        mzRange = (min(newSpectra['m/z']), max(newSpectra['m/z']))


    # min and max value of the x axis
    min_xaxis = min(newSpectra['m/z'])
    max_xaxis = max(newSpectra['m/z'])


    input_df = pd.DataFrame(newSpectra)
    ...
    plotheight_mm = plotwidth_mm * 0.6

    hspace_mm = 0.1  # horizontal space between two plots

    # read xy input data original
    '''
    raw_values = pd.read_csv(input_df, sep=" ", names=["m/z", "intensity"],
                             dtype={"m/z": np.float64, "intensity": np.float64})
    '''
    # read input data from existing dataframe
    raw_values = input_df.astype({"m/z": np.float64, "intensity": np.float64})
    ...
    # input_annotations = input_annotations.iloc[::-1]

    caption_values = input_annotations.astype({"m/z": np.float64})
    ...
    for i in caption_values.index:
        raw_values_index = raw_values[raw_values["m/z"] == caption_values.iloc[i]["m/z"]].index[0]
        value, index = float(raw_values.loc[raw_values_index - 5, "intensity_normalized"]), raw_values_index - 5
