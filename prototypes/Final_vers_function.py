...
def create_fit_df(pep_index):
    ...
    def dfs(cur_peak, aa_path, peak_path, all_paths, mz_round, df_aa):
        ...
        if cur_peak == mz_round[-1]:
            next_peak = mz_round[-2]
            dfs(
                next_peak,
                [],
                [next_peak],
    ...
    def intensity_rank(peaks_list):
        ...
        return rank_sum
    #%%
    def simulate_rank_sum_distribution(N, T, num_samples=100000):
        rank_sums = []
    ...
    # Efficient intensity rank scoring
    def intensity_rank(peaks_list):
        rank_sum = 0
        for i in peaks_list:
            intensity = intensity_peaks[np.where(mz_round == i)[0][0]]
    ...
    pep_df["rank_sum"] = [intensity_rank(p) for p in pep_df["peaks"]]

    # Fast empirical p-value using searchsorted
    def fast_empirical_p(observed, sorted_distribution):
        return np.searchsorted(sorted_distribution, observed, side='right') / len(sorted_distribution) + 0.000001

    # Pre-sort distributions
    sorted_rank_distributions = {k: np.sort(v) for k, v in rank_distributions.items()}
    pep_df["rank_p"] = [fast_empirical_p(r, sorted_rank_distributions[l]) for r, l in zip(pep_df["rank_sum"], pep_df["len_pep"])]
    ...
    # SSE scoring loop
    sse_list = []
    for row in pep_df.itertuples():
        sse_list.append(find_SSE(row.sequence, row.peaks))
    pep_df["SSE"] = sse_list

    # SSE p-value
    sorted_sse_distributions = {k: np.sort(v) for k, v in SSE_distributions.items()}
    pep_df["mz_fid_p"] = [fast_empirical_p(sse, sorted_sse_distributions[l]) for sse, l in zip(pep_df["SSE"], pep_df["len_pep"])]
    ...
    pep_df.sort_values(by=["combined_p"], ascending=True).reset_index().to_csv(f"../fits_2/{peptide}_fits.csv")
    print("pep_df saved")
